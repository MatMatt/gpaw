...
def test_run(ndim, nspin, nkpt, unocc, dtype, gpaw_new):
    ...
    for kpt in range(nkpt):
        for spin in range(nspin):
            if unocc == 0:
                skewhermitian_rotation_full(ndim, nocc, dtype)
            else:
                skewhermitian_rotation_full(ndim, nocc, dtype)
                skewhermitian_rotation_uinvar(ndim, nocc, dtype)
                skewhermitian_rotation_sparse(ndim, nocc, dtype)


def skewhermitian_rotation_sparse(ndim, nocc, dtype):
    """
    ...
    vec_len = ((ndim - nocc) * nocc) + (nocc * (nocc - 1)) // 2
...
# Step 2: Manually reconstruct full skew-Hermitian matrix
    ...
    ind_up = np.triu_indices(nocc, 1, ndim)
    ...
    sk = SkewHermitian(ndim, nocc, dtype, representation="sparse",
                       data=param_vec)
...
def skewhermitian_rotation_uinvar(ndim, nocc, dtype):
    """
    Test that SkewHermitian rotation_mat correctly computes
    the matrix exponential of a manually reconstructed skew-Hermitian matrix.
    ...
    ind_up_uinv1, ind_up_uinv2 = np.indices((nocc, (ndim - nocc)))
    ind_up = ((np.concatenate(ind_up_uinv1)).tolist(),
              (np.concatenate(ind_up_uinv2 + nocc)).tolist())
    ...
    # Step 4: Use SkewHermitian class
    sk = SkewHermitian(ndim, nocc, dtype, representation="u-invar",
                       data=param_vec)
    U_class = sk.rotation_mat
...
def skewhermitian_rotation_full(ndim, nocc, dtype):
    """
    Test that SkewHermitian rotation_mat correctly computes
    the matrix exponential of a manually reconstructed skew-Hermitian matrix.
    Representation = "full"
    ...
    # Step 4: Use SkewHermitian class
    sk = SkewHermitian(ndim, nocc, dtype, representation="full",
                       data=param_vec)
    U_class = sk.rotation_mat
