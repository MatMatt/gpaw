...
def main():
    ...
    parser.add_option(
        "-m", "--mail", dest='mail',
        help='Where to send an email about starting/ending of the job '
        '(def: read from environment variable GPAW_MAIL)')
    parser.add_option(
        "-M", "--Memory", action='count', default=None,
    ...
    if 'parameters' in set:
        ...
        for replace in [' ', os.sep]:
            parameters = parameters.replace(replace, '_')
        parameter_ext += '_' + parameters
