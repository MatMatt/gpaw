...
For this part of the project, we will move on to a magnetic monolayer in which the dominant exchange coupling is antiferromagnetic, namely
VI$_2$. We will still use the localized spin Hamiltonian.
...
Since we will need to do LDA calculations later on, we will start of this part of the project by relaxing the atomic structure of VI$_2$
...
In the cell above, we not only performed a structural optimization, but we also took the relaxed structure, switched off the $k$-point
...
To realize that VI$_2$ is in fact an antiferromagnetic material, we need to do a calculation starting from an antiferromagnetic alignment of
the spins. To do so, we need more than a single V atom in the unit cell. In the cell below, the atomic structure is repeated once to obtain
...
output of the antiferromagnetic and ferromagnetic calculations from the cell above.
...
3.   Verify that the Heisenberg model with classical spins gives a lower energy with this configuration than in the antialigned structure
     calculated above.
...
1.   Fill in the cell below with code to compute the nearest neighbour exchange coupling $J$ based on the noncollinear calculations
     performed above.
2.   Fill in code that computes the spin-orbit coupling corrected energies of the ferromagnetic state from the noncollinear calculation
     with spins directed along the $x$, $y$ and $z$ directions (Hint: the anisotropy is calculated by rotating the entire initial spin
     configuration first by $\theta$ and then by $\varphi$).
...
120$^\circ$ noncollinear magnetic order. Again, bear in mind that all the calculations in the present notebook ought to be properly
converged with respect to $k$-points, plane wave cut-off etc. to achieve an accurate estimate of e.g. the in-plane anisotropy.

Clearly the noncollinear spin state of VI$_2$ is more difficult to describe than the ferromagnetic state in CrI$_3$ and we do not yet have
